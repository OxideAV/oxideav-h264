...
/// Round-456 — §8.7 picture-level deblocking on **>8-bit** progressive
/// reconstruction planes (samples as `i32`, bit depths per plane):
/// the same `MbGrid` construction as [`deblock_recon`], run through the
/// decoder's bit-depth-aware `deblock_picture_full` (Table 8-16/8-17
/// α / β / tC0 scaled by `1 << (BitDepth − 8)`, §8.7.2.2). Chroma
/// arrays follow `chroma_array_type` (1 / 2 / 3).
#[allow(clippy::too_many_arguments)]
pub fn deblock_recon_deep(
    width: u32,
    height: u32,
    recon_y: &mut [i32],
    recon_u: &mut [i32],
    ...
    let width_mbs = width / 16;
    let height_mbs = height / 16;
    debug_assert_eq!(mb_infos.len(), (width_mbs as usize) * (height_mbs as usize));
