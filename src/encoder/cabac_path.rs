...
use crate::encoder::transform::{
    ...
    forward_hadamard_4x4, quantize_4x4_ac_w, quantize_8x8_w, quantize_chroma_dc_w,
...
struct CabacEncMbInfo {
    ...
    cbf_cb_dc: bool,
    cbf_cr_dc: bool,
    /// Chroma AC CBF per 4x4 block — 4 blocks at 4:2:0, 8 at 4:2:2
    /// (2x4 raster layout).
    cbf_cb_ac: [bool; 8],
    cbf_cr_ac: [bool; 8],
...
fn cbf_neighbour_chroma_ac(
    ...
    // Chroma block layout: 2x2 blocks at 4:2:0, 2x4 at 4:2:2.
    // Coordinates in 4-pixel units.
    ...
    let pick = |xn: i32, yn: i32, is_left: bool| -> bool {
        if xn >= 0 && yn >= 0 {
            ...
            // Wrap into the neighbour's chroma tile (§9.3.3.1.1.9
            // cat-4 → §6.4.11.5): height 8 at 4:2:0, 16 at 4:2:2 —
            // decoder mirror (`cbf_cond_for` InternalChromaAc).
...
fn quantize_intra16x16_luma(
    ...
    let dc_levels = quantize_luma_dc_w(&dc_hadamard, qp_y, true, w4[0]);
    ...
    for blkz in 0..16usize {
        ...
        let raster = by * 4 + bx;
        let mut z = quantize_4x4_ac_w(&coeffs_4x4[raster], qp_y, true, w4);
...
fn reconstruct_intra16x16_luma(
    ...
    for blk in 0..16usize {
        ...
        let r = inverse_transform_4x4_dc_preserved(&coeffs_in, qp_y, w4, 8).expect("inverse 4x4");
        for j in 0..4 {
            for i in 0..4 {
...
fn intra16x16_luma_cost_estimate(
    ...
    reconstruct_intra16x16_luma(
        mb_x, mb_y, width, pred, dc, ac_quant, cbp_luma, qp_y, recon_y, w4,
    );
...
fn trial_intra8x8_for_cabac(
    ...
    for blk8 in 0..4usize {
        ...
        for mode_idx in 0..9u8 {
            ...
            let z_raster = quantize_8x8_w(&w_coeffs, qp_y, true, w8);
            let r = inverse_transform_8x8(&z_raster, qp_y, w8, 8).expect("inverse 8x8");
...
/// §8.3.4.5 / §7.3.5.3 — encode one chroma plane of a 4:4:4 Intra_8x8
/// MB under the "coded like luma" rule: each 8x8 quadrant reuses the
/// luma-chosen Intra_8x8 prediction mode on the chroma plane, runs the
/// §8.6.4 forward 8x8 transform + §8.5.13.1 quantiser at the chroma QP
/// and commits the normative inverse into the plane recon. Returns the
/// Table 8-14 scan list + non-zero flag per quadrant (the blockCat-9/13
/// CABAC residual payload). Mirrors the CAVLC `encode_mb_intra8x8_444`
/// chroma half.
fn chroma_plane_i8x8_444_cabac(
    plane_src: &[u8],
    plane_recon: &mut [u8],
    width: usize,
    mb_x: usize,
    ...
    let sl8 = *w8;
    let mut scan8 = [[0i32; 64]; 4];
    let mut has_nz = [false; 4];
    for blk8 in 0..4usize {
        let bx = mb_x * 16 + (blk8 % 2) * 8;
        let by = mb_y * 16 + (blk8 / 2) * 8;
        let mode = Intra8x8Mode::from_index(luma_modes[blk8]).expect("mode index in range");
        let raw = gather_8x8_from_recon(plane_recon, width, mb_x, mb_y, blk8);
        let filtered = filter_samples_8x8(&raw, 8);
        let mut pred = [0i32; 64];
        predict_8x8(mode, &filtered, 8, &mut pred);
        let mut res = [0i32; 64];
        ...
        scan8[blk8] = zigzag_scan_8x8(&z_raster);
        has_nz[blk8] = z_raster.iter().any(|&v| v != 0);
    }
    (scan8, has_nz)
}

/// §9.3.3.1.1.9 cat 5 / 9 / 13 — coded_block_flag cond terms for an
/// 8x8 transform block (encoder mirror of the decoder's
/// `CbfLoc::Internal8x8` arm). transBlockN is assigned only when the
/// neighbour MB used `transform_size_8x8_flag == 1` AND its CBP bit
/// covers the wrapped 8x8 index; the 8x8 CBF is folded into the four
/// per-plane 4x4 slots (top-left slot carries the value).
fn cbf_neighbour_8x8(
    grid: &CabacEncGrid,
    mb_x: usize,
    mb_y: usize,
    current: &CabacEncMbInfo,
    ...
    let folded = |info: &CabacEncMbInfo, b8: usize| -> bool {
        let bi = b8 * 4;
        match block_type {
            BlockType::Luma8x8 => info.cbf_luma_4x4.get(bi).copied().unwrap_or(false),
            BlockType::Cb8x8 => info.cbf_cb_ac_444.get(bi).copied().unwrap_or(false),
            BlockType::Cr8x8 => info.cbf_cr_ac_444.get(bi).copied().unwrap_or(false),
            _ => false,
        }
    };
    let x8 = (blk8 % 2) as i32;
    let y8 = (blk8 / 2) as i32;
    let pick = |xn: i32, yn: i32, is_left: bool| -> bool {
        if xn >= 0 && yn >= 0 {
            // Internal — the current MB's progressive folded state
            // (CBP-bit-0 quadrants stay false, matching the spec's
            // "transBlockN not available → 0" rule).
            folded(current, (yn * 2 + xn) as usize)
        } else {
            let nbr = if is_left {
                grid.left(mb_x, mb_y)
            } else {
            ...
            let (wx8, wy8) = if is_left { (xn + 2, yn) } else { (xn, yn + 2) };
            let blk8n = (wy8 * 2 + wx8) as usize;
            if ((info.cbp_luma >> blk8n) & 1) == 0 || !info.transform_size_8x8 {
                return false;
            }
            folded(info, blk8n)
        }
    };
    (pick(x8 - 1, y8, true), pick(x8, y8 - 1, false))
}
...
fn encode_chroma_intra16x16_420(
    ...
    let dc_levels = quantize_chroma_dc_w(&dc_had, qp_c, true, w4[0]);
...
impl Encoder {
    ...
    pub fn encode_idr_cabac(&self, frame: &YuvFrame<'_>) -> EncodedIdr {
        ...
        // refinements still assume flat dequantisation.
        if !cfg.scaling_matrix.is_flat() {
            assert!(
                !cfg.trellis_quant_intra && !cfg.trellis_quant_intra_chroma,
                "trellis intra refinement assumes flat scaling lists",
            );
            if let crate::encoder::ScalingMatrixMode::SeqCustom(l)
            | crate::encoder::ScalingMatrixMode::PicCustom(l) = cfg.scaling_matrix
            {
                assert!(
                    l.is_valid(),
        ...
        // group / §7.3.2.2 PPS tail (High-family syntax).
        let profile_idc =
            if (cfg.transform_8x8 || !cfg.scaling_matrix.is_flat()) && cfg.profile_idc < 100 {
                100
            } else {
                cfg.profile_idc
            };
        ...
        // reduces to the legacy all-16 arithmetic.
        let wq_i = cfg.scaling_matrix.intra_weights();
        ...
        for mb_y in 0..height_mbs {
            for mb_x in 0..width_mbs {
                ...
                // Pick intra modes. 4:2:0 picks the chroma mode here;
                // 4:2:2 delegates to `encode_chroma_block` below (8x16
                // tile geometry); 4:4:4 reuses the luma mode (§8.3.4.1).
                let (luma_mode, luma_pred) =
                    pick_intra16x16_mode(frame, &recon_y, width, height, mb_x, mb_y);
                let mut chroma_mode = IntraChromaMode::Dc;
                if cfg.chroma_format_idc == 1 {
                    let (m, _, _) = pick_chroma_mode(
                ...
                let (luma_dc, luma_ac_quant, luma_ac_scan, any_luma_ac_nz) =
                    quantize_intra16x16_luma(&residual, qp_y, cfg.trellis_quant_intra, &wq_i.w4);
                ...
                // 4:4:4 Intra_8x8 chroma residual (blockCat-9/13 payload;
                // only populated when fmt==3 AND the MB is Intra_8x8).
                let mut cb_scan8_444 = [[0i32; 64]; 4];
                let mut cr_scan8_444 = [[0i32; 64]; 4];
                let mut cb8_nz_444 = [false; 4];
                let mut cr8_nz_444 = [false; 4];
                ...
                // 4:2:2 chroma block (only populated when fmt==2).
                let mut chroma_422: Option<super::ChromaBlock> = None;
                let cbp_chroma: u8;

                if cfg.chroma_format_idc == 2 {
                    // 4:2:2 — §8.5.11.2 chain (mode pick + 4x2-Hadamard
                    ...
                    chroma_422 = Some(cbk);
                } else if cfg.chroma_format_idc == 3 && i8x8_cand.is_some() {
                    // 4:4:4 Intra_8x8 — §8.3.4.5 chroma coded like luma
                    // with the 8x8 transform at the chroma QP, reusing
                    // the per-quadrant luma-chosen modes.
                    let luma_modes = intra_grid.slot(mb_x, mb_y).intra_8x8_pred_modes;
                    let (cbs, cbnz) = chroma_plane_i8x8_444_cabac(
                        frame.u,
                        &mut recon_u,
                        chroma_w,
                        mb_x,
                    ...
                    cb_scan8_444 = cbs;
                    cb8_nz_444 = cbnz;
                    let (crs, crnz) = chroma_plane_i8x8_444_cabac(
                        frame.v,
                        &mut recon_v,
                        chroma_w,
                        mb_x,
                    ...
                    let recon_cb_res = if cbp_chroma == 2 {
                        ...
                        chroma_residual_dc_only(&cb_dc_420, qp_c, &wq_i.w4)
                    } else {
                        [0i32; 64]
                    };
                    let recon_cr_res = if cbp_chroma == 2 {
                        ...
                        chroma_residual_dc_only(&cr_dc_420, qp_c, &wq_i.w4)
                    } else {
                        [0i32; 64]
                    };
                    for j in 0..8usize {
                ...
                let cbp_luma_i8x8: u8 = i8x8_cand
                    ...
                    .map(|c| {
                        ...
                        for (b, &nz) in c.blk8_has_nz.iter().enumerate() {
                            // §7.4.5.1 at ChromaArrayType == 3: a
                            // quadrant's CBP bit covers all three planes.
                            if nz
                                || (cfg.chroma_format_idc == 3 && (cb8_nz_444[b] || cr8_nz_444[b]))
                            {
                                v |= 1 << b;
                            }
                        }
                ...
                if let Some(c8) = &i8x8_cand {
                    ...
                    // The chroma TU suffix is emitted only at
                    // ChromaArrayType 1 / 2 (§9.3.3.1.1.4).
                    ...
                    // quadrant. At ChromaArrayType != 3 coded_block_flag
                    // is suppressed (inferred from CBP); at 4:4:4 the CBF
                    // IS coded with the §9.3.3.1.1.9 cat-5 cond terms.
                    // Either way the value folds into the four 4x4 slots
                    // for downstream neighbour reads (decoder mirror).
                ...
                // Chroma residual — 4:2:0/4:2:2 or 4:4:4 path.
                if cfg.chroma_format_idc == 3 && i8x8_cand.is_some() {
                    // §7.3.5.3 — each chroma plane repeats the luma
                    // residual layout: one blockCat-9 (Cb) / blockCat-13
                    // (Cr) 64-coefficient block per coded quadrant, CBF
                    // explicitly coded (§7.3.5.3.3 at ChromaArrayType 3)
                    // and folded into the per-plane 4x4 slots.
                    for (bt, scans) in [
                        (BlockType::Cb8x8, &cb_scan8_444),
                        (BlockType::Cr8x8, &cr_scan8_444),
                    ] {
                        for blk8 in 0..4usize {
                            if (cbp_luma_i8x8 >> blk8) & 1 == 1 {
                                let cur = grid.at(mb_x, mb_y).clone();
                                let (ca, cb) =
                                    cbf_neighbour_8x8(&grid, mb_x, mb_y, &cur, true, bt, blk8);
                                let coded = encode_residual_block_cabac(
                                    &mut cabac,
                                    &mut ctxs,
                                    bt,
                    ...
                    // ChromaACLevel covers 4 / 8 blocks per plane.
                    let num_c8x8: u32 = if cfg.chroma_format_idc == 2 { 2 } else { 1 };
                    let n_dc = (4 * num_c8x8) as usize;
                    let dc_cb_slice: &[i32] = match &chroma_422 {
                        Some(cbk) => &cbk.dc_cb[..n_dc],
                        None => &cb_dc_420[..],
                    };
                    let dc_cr_slice: &[i32] = match &chroma_422 {
                        Some(cbk) => &cbk.dc_cr[..n_dc],
                        None => &cr_dc_420[..],
                    };
                    if cbp_chroma > 0 {
                        let (cb_a, cb_b) =
                    ...
                    if cbp_chroma == 2 {
                        for blk in 0..n_dc {
                            let ac: &[i32] = match &chroma_422 {
                                Some(cbk) => &cbk.ac_cb[blk][..15],
                                None => &cb_ac_scan_420[blk][..15],
                            };
                            let cur = grid.at(mb_x, mb_y).clone();
                        ...
                        for blk in 0..n_dc {
                            let ac: &[i32] = match &chroma_422 {
                                Some(cbk) => &cbk.ac_cr[blk][..15],
                                None => &cr_ac_scan_420[blk][..15],
                            };
                            let cur = grid.at(mb_x, mb_y).clone();
                            let (ca, cb) = cbf_neighbour_chroma_ac(
                                &grid,
        ...
        for mb_y in 0..height_mbs {
            for mb_x in 0..width_mbs {
                ...
                for blkz in 0..16usize {
                    ...
                    let mut q =
                        crate::encoder::transform::quantize_4x4_w(&coeffs, qp_y, false, &wq.w4);
                    ...
                    let r = crate::transform::inverse_transform_4x4(&q, qp_y, &wq.w4, 8)
                        .expect("inverse 4x4");
                    for j in 0..4 {
                        for i in 0..4 {
    ...
    pub fn encode_b_cabac(
        ...
        // Round-391 — non-flat scaling matrices run on the CABAC B
        // path at 4:2:0 (see encode_p_cabac).
        let wq = cfg.scaling_matrix.inter_weights();
        assert!(
            cfg.profile_idc >= 77,
        ...
        for mb_y in 0..height_mbs {
            for mb_x in 0..width_mbs {
                ...
                if cfg.chroma_format_idc != 1 {
                    ...
                    let sad_of = |pred: &[i32; 256]| -> u64 {
                        let mut d = 0u64;
                        for j in 0..16usize {
                            for i in 0..16usize {
                                let sv = frame.y[(mb_y * 16 + j) * width + mb_x * 16 + i] as i32;
                                d += sv.abs_diff(pred[j * 16 + i]) as u64;
                            }
                        }
                        d
                    };
                    ...
                    let build_c = |plane: &[u8], mv: Mv| -> Vec<i32> {
                        match cfg.chroma_format_idc {
                            ...
                            .to_vec(),
                            _ => build_inter_pred_chroma_local_422(
                                plane,
                                chroma_w as u32,
                                chroma_h as u32,
                                mb_x,
                    ...
                    let (pred_u, pred_v): (Vec<i32>, Vec<i32>) = match eff_kind {
                        ...
                        _ => {
                            ...
                            for k in 0..tile {
                                u[k] = ((u0[k] + u1[k] + 1) >> 1).clamp(0, 255);
                                v[k] = ((v0[k] + v1[k] + 1) >> 1).clamp(0, 255);
                            }
                            (u, v)
                        }
                    };
                    ...
                    let mut cbp_luma: u8 = 0;
                    for blk8 in 0..4usize {
                        if (0..4).any(|sub| fwd4.blk_has_nz[blk8 * 4 + sub]) {
                            cbp_luma |= 1u8 << blk8;
                        }
                    }
                    let mut transform_size_8x8 = false;
                    ...
                    if cfg.transform_8x8 {
                        ...
                        let mut cbp_luma8: u8 = 0;
                        for blk8 in 0..4usize {
                            if fwd8.blk_has_nz[blk8 * 4] {
                                cbp_luma8 |= 1u8 << blk8;
                            }
                        }
                        let lambda = lambda_ssd(qp_y, false);
                        let j4 = inter_luma_transform_cost(
                            frame.y, width, mb_x, mb_y, &pred_y, &fwd, cbp_luma, lambda,
                        );
                        let j8 = inter_luma_transform_cost(
                            frame.y, width, mb_x, mb_y, &pred_y, &fwd8, cbp_luma8, lambda,
                        );
                        if j8 < j4 {
                            transform_size_8x8 = true;
                            cbp_luma = cbp_luma8;
                    ...
                    // Mark the slot for in-MB neighbour reads.
                    {
                        let cur = grid.at_mut(mb_x, mb_y);
                        cur.is_intra = false;
                        cur.is_skip = false;
                    ...
                    emit_inter_luma_residual_cabac(
                        &mut cabac,
                        &mut ctxs,
                        &mut grid,
                        mb_x,
                    ...
                    emit_inter_chroma_residual_cabac(
                        &mut cabac,
                        &mut ctxs,
                        &mut grid,
                        mb_x,
                    ...
                    for blk8 in 0..4usize {
                        ...
                        for sub in 0..4usize {
                            ...
                            for j in 0..4usize {
                                for i in 0..4usize {
                                    let pv = pred_y[(by * 4 + j) * 16 + bx * 4 + i];
                                    let r = if send {
                                        fwd.recon_residual[blkz][j * 4 + i]
                                    } else {
                                        0
                                    ...
                                    let py = mb_y * 16 + by * 4 + j;
                                    let px = mb_x * 16 + bx * 4 + i;
                                    recon_y[py * width + px] = (pv + r).clamp(0, 255) as u8;
                                }
                            }
                        }
                    }
                    if cfg.chroma_format_idc == 3 {
                        for (cfwd, cpred, recon) in [
                            (&cb444, &pred_u, &mut recon_u),
                            (&cr444, &pred_v, &mut recon_v),
                        ] {
                            for blk8 in 0..4usize {
                                ...
                                for sub in 0..4usize {
                                    ...
                                    for j in 0..4usize {
                                        for i in 0..4usize {
                                            let pv = cpred[(by * 4 + j) * 16 + bx * 4 + i];
                                            let r = if send {
                                                cfwd.recon_residual[blkz][j * 4 + i]
                                            } else {
                                                0
                                            };
                                            let cy = mb_y * 16 + by * 4 + j;
                                            let cx = mb_x * 16 + bx * 4 + i;
                                            recon[cy * chroma_w + cx] =
                                                (pv + r).clamp(0, 255) as u8;
                                        }
                                    }
                                }
                            }
                    ...
                    let chroma_nonzero_4x4: u16 = if cfg.chroma_format_idc == 2 {
                        let mut mask = 0u16;
                        if cbp_chroma == 2 {
                            for k in 0..8usize {
                                if cb_ac8[k].iter().any(|&v| v != 0) {
...
/// Round-391 — emit one inter MB's **luma** residual (shared by the
/// multi-format P and B CABAC paths). 4x4 blocks go out as blockCat-2
/// with explicit CBF; 8x8 quadrants as blockCat-5 with CBF suppressed
/// at ChromaArrayType != 3 and explicitly coded (§9.3.3.1.1.9 cat-5
/// cond terms) at 4:4:4. Folds the CBF values into the grid slots for
/// downstream neighbour reads.
#[allow(clippy::too_many_arguments)]
fn emit_inter_luma_residual_cabac(
    ...
    scan8_blocks: &[[i32; 64]; 4],
) {
    let cbf_coded_444 = chroma_format_idc == 3;
    if transform_size_8x8 {
        for blk8 in 0..4usize {
            if (cbp_luma >> blk8) & 1 == 1 {
                let (ca, cb) = if cbf_coded_444 {
                    let cur = grid.at(mb_x, mb_y).clone();
                    let (a, b) =
                        cbf_neighbour_8x8(grid, mb_x, mb_y, &cur, false, BlockType::Luma8x8, blk8);
                    (Some(a), Some(b))
                } else {
                    (None, None)
                };
...
/// Round-391 — emit one inter MB's **chroma** residual for any chroma
/// format (shared by the multi-format P and B CABAC paths):
///   * 4:2:0 / 4:2:2 — ChromaDCLevel (4 / 8 coefficients, NumC8x8-aware
///     eq. (9-22) contexts) + per-plane ChromaACLevel blocks.
///   * 4:4:4 — §7.3.5.3 chroma coded like luma: blockCat-9/13 8x8
///     residuals (explicit CBF) under transform_size_8x8, blockCat-8/12
///     full 4x4 residuals otherwise, gated by the shared cbp_luma.
#[allow(clippy::too_many_arguments)]
fn emit_inter_chroma_residual_cabac(
    cabac: &mut CabacEncoder,
    ctxs: &mut CabacContexts,
    grid: &mut CabacEncGrid,
    mb_x: usize,
    ...
    cb_ac: &[[i32; 16]],
    cr_ac: &[[i32; 16]],
    cb444: &InterLumaForward,
    cr444: &InterLumaForward,
    cb444_scan8: &[[i32; 64]; 4],
    cr444_scan8: &[[i32; 64]; 4],
) {
            ...
            if transform_size_8x8 {
                for blk8 in 0..4usize {
                    if (cbp_luma >> blk8) & 1 == 1 {
                        let cur = grid.at(mb_x, mb_y).clone();
                        let (a, b) =
                            cbf_neighbour_8x8(grid, mb_x, mb_y, &cur, false, blk8_type, blk8);
                        let coded = encode_residual_block_cabac(
                            cabac,
                            ctxs,
                            blk8_type,
    ...
    // 4:2:0 / 4:2:2.
    let num_c8x8: u32 = if chroma_format_idc == 2 { 2 } else { 1 };
    let n_dc = (4 * num_c8x8) as usize;
    if cbp_chroma > 0 {
        let (cb_a, cb_b) = cbf_neighbour_mb_level(grid, mb_x, mb_y, false, CbfPlane::CbDc);
        let cb_dc_coded = encode_residual_block_cabac(
            cabac,
            ctxs,
            BlockType::ChromaDc,
        ...
        grid.at_mut(mb_x, mb_y).cbf_cb_dc = cb_dc_coded;
        let (cr_a, cr_b) = cbf_neighbour_mb_level(grid, mb_x, mb_y, false, CbfPlane::CrDc);
        let cr_dc_coded = encode_residual_block_cabac(
            cabac,
            ctxs,
            BlockType::ChromaDc,
    ...
    if cbp_chroma == 2 {
        for (is_cr, ac) in [(false, cb_ac), (true, cr_ac)] {
            for blk in 0..n_dc {
                ...
                let info = grid.at_mut(mb_x, mb_y);
                if is_cr {
                    info.cbf_cr_ac[blk] = coded;
                } else {
                    info.cbf_cb_ac[blk] = coded;
                }
            }
        }
    }
...
fn encode_chroma_inter_420(
    ...
    let dc_in = [blocks[0][0], blocks[1][0], blocks[2][0], blocks[3][0]];
    let dc_had = forward_hadamard_2x2(&dc_in);
    let dc_levels = quantize_chroma_dc_w(&dc_had, qp_c, false, w4[0]);
    let mut ac_scan = [[0i32; 16]; 4];
    let mut ac_quant = [[0i32; 16]; 4];
    let mut any_ac_nz = false;
    for blk in 0..4usize {
        let z = quantize_4x4_ac_w(&blocks[blk], qp_c, false, w4);
        ac_quant[blk] = z;
        let s = zigzag_scan_4x4_ac(&z);
        ac_scan[blk] = s;
        if s.iter().any(|&v| v != 0) {
    ...
    let any_dc_nz = dc_levels.iter().any(|&v| v != 0);
    let inv_dc = inverse_hadamard_chroma_dc_420(&dc_levels, qp_c, w4, 8).unwrap();
    ...
    for blk in 0..4usize {
        ...
        let mut coeffs_in = ac_quant[blk];
        coeffs_in[0] = inv_dc[blk];
        let r = inverse_transform_4x4_dc_preserved(&coeffs_in, qp_c, w4, 8).unwrap();
        for j in 0..4 {
            for i in 0..4 {
...
fn encode_chroma_intra16x16_444(
    ...
    use crate::encoder::transform::{forward_core_4x4, forward_hadamard_4x4, quantize_luma_dc_w};
