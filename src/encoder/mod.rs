...
/// See the note on [`Encoder::encode_p_mb`]: decoder-visible QP_Y
/// chain state for §7.4.5 `mb_qp_delta` emission.
#[derive(Debug, Clone, Copy)]
struct MbQpTracker {
    /// QP_Y the decoder holds after the most recent MB (the §7.4.5
...
use crate::encoder::deblock::{
    chroma_nz_mask_from_blocks, luma_nz_mask_from_blocks, MbDeblockInfo,
};
...
use crate::encoder::macroblock::{
    ...
    write_intra16x16_mb_in_inter_slice, write_p_8x8_all_pl08x8_mb, B16x16McbConfig,
    B8x8MixedMcbConfig, BPartPred, BPred16x16, BSubMbCell, I16x16McbConfig, I8x8McbConfig,
...
impl Encoder {
    ...
    pub fn encode_idr_with_qp(&self, frame: &YuvFrame<'_>, frame_qp: i32) -> EncodedIdr {
        ...
        // format: the 4:2:2 §8.5.11.2 chroma chain and the 4:4:4
        // coded-like-luma planes quantise / reconstruct under the
        // §8.5.9 weightScale; the SPS/PPS writers emit the 12-list
        // loop at 4:4:4. The trellis refinements still assume flat
        // dequantisation.
    ...
    fn encode_mb_intra8x8_444(
        ...
        let width = self.cfg.width as usize;
        let lambda = lambda_ssd(qp_y, true);
        // §8.5.9 — round-397: the 8x8 quantise / reconstruct chain
        // honours the intra weightScale (flat reduces bit-exactly).
        let wq = self.cfg.scaling_matrix.intra_weights();
        let sl8 = wq.w8;
        ...
        for blk8 in 0..4usize {
            ...
            for mode_idx in 0..9u8 {
                ...
                let w_coeffs = forward_core_8x8(&res);
                let z_raster = quantize_8x8_w(&w_coeffs, qp_y, true, &sl8);
                let r = inverse_transform_8x8(&z_raster, qp_y, &sl8, 8).expect("inverse 8x8");
                let mut recon_blk = [0i32; 64];
                let mut d: u64 = 0;
                for j in 0..8 {
            ...
            for plane_is_cr in [false, true] {
                ...
                let w_coeffs = forward_core_8x8(&res);
                let z_raster = quantize_8x8_w(&w_coeffs, qp_c, true, &sl8);
...
/// Round-397 — §9.2.1.1 (ChromaArrayType == 3) per-plane nC for an
/// **inter** MB's coded-like-luma chroma plane: 16 full 4x4 blocks
/// (no separate plane DC), each block's TotalCoeff committed
...
fn derive_plane_nc_444_inter(
    ...
    for blk in 0..16usize {
        ...
        ac_nc[blk] = crate::macroblock_layer::derive_nc_plane_luma_like(
            nc_grid, mb_addr, blk as u8, is_cr, false, false,
        );
        own[blk] = if (cbp_luma >> (blk / 4)) & 1 == 1 {
            levels_scan[blk].iter().filter(|&&v| v != 0).count() as u8
        } else {
            0
        };
    }
...
fn encode_chroma_block(
    ...
    if chroma_array_type == 1 {
        ...
        let (u_dc, u_ac, u_recon) = encode_chroma_residual_422(
            frame_u,
            chroma_width,
            mb_x,
            mb_y,
        ...
        let (v_dc, v_ac, v_recon) = encode_chroma_residual_422(
            frame_v,
            chroma_width,
            mb_x,
            mb_y,
...
fn encode_intra16x16_chroma_plane_luma_like(
    ...
    let dc_levels = quantize_luma_dc_w(&dc_t, qp_c, true, w4[0]);
    ...
    for blkz in 0..16usize {
        let (bx, by) = LUMA_4X4_BLK[blkz];
        let raster = by * 4 + bx;
        let z = quantize_4x4_ac_w(&coeffs_4x4[raster], qp_c, true, w4);
    ...
    let inv_dc =
        inverse_hadamard_luma_dc_16x16(&dc_levels, qp_c, w4, 8).expect("4:4:4 chroma DC inverse");
    #[allow(clippy::needless_range_loop)] // raster row-major over 16 4x4 blocks
    for blk in 0..16usize {
        let bx = blk % 4;
        let by = blk / 4;
        ...
        coeffs_in[0] = inv_dc[by * 4 + bx];
        let r = inverse_transform_4x4_dc_preserved(&coeffs_in, qp_c, w4, 8)
...
fn encode_chroma_residual_422(
    ...
    use crate::encoder::transform::{
        forward_hadamard_4x2, quantize_4x4_ac_w, quantize_chroma_dc_422_w, scan_chroma_dc_422,
    };
    ...
    // Forward 4x2 Hadamard + quantize. §8.5.11.2 — the DC scaling
    // uses LevelScale4x4(qP % 6, 0, 0) of the chroma list (round-397:
    // non-flat weightScale honoured; a flat list reduces bit-exactly).
    let dc_t = forward_hadamard_4x2(&dc_in);
    let dc_q = quantize_chroma_dc_422_w(&dc_t, qp_c, true, w4[0]);
    ...
    let dc_back =
        inverse_hadamard_chroma_dc_422(&dc_levels, qp_c, w4, 8).expect("chroma DC 422 inverse");
    ...
    for (k, &(bx, by)) in blocks_xy.iter().enumerate() {
        ...
        let r = inverse_transform_4x4_dc_preserved(&coeffs, qp_c, w4, 8)
...
impl Encoder {
    ...
    pub fn encode_p_with_qp(
        ...
        self.encode_p_impl(frame, prev, frame_num, pic_order_cnt_lsb, frame_qp, None)
    }

    /// [`encode_p_with_qp`](Self::encode_p_with_qp) plus **MB-row QP
    /// modulation** toward a whole-slice bit budget (round-420).
    ///
    /// Before each MB row (except the first) the encoder compares the
    /// bits spent so far against the pro-rata share of
    /// `frame_budget_bits` and steps the working QP by up to ±2
    /// (bounded to `frame_qp` ± 6): rows landing over budget code at
    /// a coarser QP, rows under budget claw quality back. The QP
    /// changes ride the §7.4.5 `mb_qp_delta` syntax — emitted only on
    /// MBs with coded residual, with skipped / cbp==0 MBs inheriting
    /// the previous MB's QP_Y exactly as the decoder derives it (the
    /// §8.7 deblock strengths follow the same chain).
    ///
    /// 4:2:0 only (the modulated paths' §7.3.5 mb_qp_delta gating is
    /// exercised at ChromaArrayType == 1).
    pub fn encode_p_rate_adaptive(
        &self,
        frame: &YuvFrame<'_>,
        prev: &EncodedFrameRef<'_>,
        frame_num: u32,
        ...
        assert_eq!(
            ...
            "encode_p_rate_adaptive requires 4:2:0"
        );
        self.encode_p_impl(
            frame,
            prev,
    ...
    fn encode_p_impl(
        ...
        // §8.5.8 BD-aware chroma-QP — see encode_idr; identical
        // reasoning. Derived per row when MB-row modulation is active.
        let qp_bd_offset_c = qp_bd_offset(self.cfg.bit_depth_chroma_minus8);
        ...
        for mb_y in 0..height_mbs as usize {
            // Round-420 — MB-row QP modulation: compare bits spent so
            // far to the pro-rata share of the slice budget and step
            // the working QP. +6 QP doubles the quantiser step, so
            // log2 of the overshoot ratio is the natural correction
            // unit. The response is deliberately asymmetric: running
            // out of budget mid-frame is the harmful direction, so
            // overshoot climbs fast (up to +2 per row, ceiling
            // frame_qp + 6) while undershoot descends one step per
            // row with a floor of frame_qp - 2 — early cheap rows say
            // nothing about the rows still to come (a mid-frame
            // complexity cliff would otherwise be entered at a
            // too-fine QP), and sustained underspend is reclaimed by
            // the frame-level controller on the next picture anyway.
            if let Some(budget) = row_budget_bits {
                if mb_y > 0 && budget > 0 {
                    ...
                    // Refinement below the frame QP is only trusted
                    // once most of the slice has proven cheap — a
                    // front-loaded saving is routinely an artefact of
                    // a rear-loaded complexity distribution.
        ...
        // §8.7 — picture-level in-loop deblocking. 4:4:4 skips the
        // pass (the slice header set disable_deblocking_filter_idc = 1
        // so the decoder doesn't run §8.7 either).
        if self.cfg.chroma_format_idc != 3 {
            crate::encoder::deblock::deblock_recon_with_chroma_array_type(
                self.cfg.width,
                self.cfg.height,
    ...
    #[allow(clippy::too_many_arguments)]
    /// §7.4.5 — decoder-visible QP_Y tracker for MB-level delta-QP
    /// emission. `mb_qp_delta` is only present on MBs with coded
    /// residual (inter cbp != 0, or Intra_16x16 which always codes
    /// the luma DC block); every other MB inherits the previous MB's
    /// QP_Y in decoding order. The encoder mirrors that chain so (a)
    /// the emitted delta is relative to the decoder's actual
    /// predictor, and (b) the deblock pass filters each MB at the QP
    /// the decoder will assign it (skip / cbp==0 MBs keep `cur`).
    #[doc(hidden)]
    fn encode_p_mb(
        &self,
        frame: &YuvFrame<'_>,
        ...
        //    (round-397):
        //      * 4:2:0 — 1/8-pel bilinear, mvC = mvL.
        //      * 4:2:2 — x subsampled as 4:2:0, y full height
        //        (mvC.y = mvL.y·2 in 1/8-pel units).
        ...
        let (pred_u, pred_v): (Vec<i32>, Vec<i32>) = match chroma_array_type {
            3 => (
                build_inter_pred_luma(prev.recon_u, chroma_w, chroma_h, mb_x, mb_y, chosen_mv)
                    .to_vec(),
                build_inter_pred_luma(prev.recon_v, chroma_w, chroma_h, mb_x, mb_y, chosen_mv)
                    .to_vec(),
            ),
            2 => (
                cabac_path::build_inter_pred_chroma_local_422(
                    prev.recon_u,
                    chroma_w,
                    chroma_h,
                    mb_x,
        ...
        // Round-397 — the 4MV P_8x8 writer's per-4x4 chroma predictor
        // assembly is 4:2:0-only; the 4:2:2 / 4:4:4 encoder keeps the
        // single-MV shapes (P_Skip / P_L0_16x16).
        let try_4mv = chroma_array_type == 1
            && me_16x16_sad >= 1024
            && (me_4mv_sad_sum as u64) * 10 < (me_16x16_sad as u64) * 7;
        ...
        //    per-format dispatch (round-397).
        // 4:2:0 containers.
        let mut u_dc_levels = [0i32; 4];
        let mut v_dc_levels = [0i32; 4];
        let mut u_ac_levels = [[0i32; 16]; 4];
        let mut v_ac_levels = [[0i32; 16]; 4];
        let mut u_recon_residual = [0i32; 64];
        let mut v_recon_residual = [0i32; 64];
        // 4:2:2 containers (§8.5.11.2 — 8-coefficient DC + 2x4 AC grid).
        let mut u_dc8 = [0i32; 8];
        let mut v_dc8 = [0i32; 8];
        let mut u_ac8 = [[0i32; 16]; 8];
        let mut v_ac8 = [[0i32; 16]; 8];
        let mut u_recon_res_422 = [0i32; 128];
        let mut v_recon_res_422 = [0i32; 128];
        ...
        match chroma_array_type {
            3 => {
                ...
                // §7.4.5.1 at ChromaArrayType == 3 — a CBP quadrant bit
                // covers all three planes; CodedBlockPatternChroma is 0
                // (Table 9-4(b) has no chroma column).
                for blk8 in 0..4usize {
                ...
                // recon / grids agree with the decoder.
                if cbp_luma == 0 {
            ...
            2 => {
                ...
                let (dcu, acu, ru) =
                    encode_chroma_residual_422(frame.u, chroma_width, mb_x, mb_y, pu, qp_c, &wq.w4);
                let (dcv, acv, rv) =
                    encode_chroma_residual_422(frame.v, chroma_width, mb_x, mb_y, pv, qp_c, &wq.w4);
                u_dc8 = dcu;
                v_dc8 = dcv;
                u_ac8 = acu;
                v_ac8 = acv;
                u_recon_res_422 = ru;
                v_recon_res_422 = rv;
                let any_dc_nz = u_dc8.iter().any(|&v| v != 0) || v_dc8.iter().any(|&v| v != 0);
                let any_ac_nz = u_ac8.iter().any(|b| b.iter().any(|&v| v != 0))
                    || v_ac8.iter().any(|b| b.iter().any(|&v| v != 0));
            ...
            _ => {
                ...
                let (dcu, acu, ru) = encode_chroma_residual_inter(
                    frame.u,
                    chroma_width,
                    mb_x,
                    mb_y,
                ...
                let (dcv, acv, rv) = encode_chroma_residual_inter(
                    frame.v,
                    chroma_width,
                    mb_x,
                    mb_y,
                ...
                v_recon_residual = rv;
                let any_dc_nz =
                    u_dc_levels.iter().any(|&v| v != 0) || v_dc_levels.iter().any(|&v| v != 0);
            ...
            *mv_grid.slot_mut(mb_x, mb_y) = MvGridSlot {
            ...
            for j in 0..ct_h {
                for i in 0..ct_w {
                    let cy = mb_y * ct_h + j;
                    let cx = mb_x * ct_w + i;
                    recon_u[cy * chroma_width + cx] = pred_u[j * ct_w + i].clamp(0, 255) as u8;
                    recon_v[cy * chroma_width + cx] = pred_v[j * ct_w + i].clamp(0, 255) as u8;
                }
            }
            let _ = chroma_height;
            ...
            return MbDeblockInfo {
                is_intra: false,
                // §7.4.5 — a skipped MB carries no mb_qp_delta; its
                // QP_Y (used by the §8.7 deblock strength derivation)
                // is the decoder's running QP, not this row's target.
                qp_y: qp_tracker.cur,
                luma_nonzero_4x4: 0,
        ...
        *pending_skip = 0;
        ...
        match chroma_array_type {
            3 => {
                ...
                // the luma loop above).
                for (pred, fwd, recon) in [
                    (&pred_u, &cb444, &mut *recon_u),
                    (&pred_v, &cr444, &mut *recon_v),
                ] {
                    for blk8 in 0..4usize {
                        let send = (cbp_luma >> blk8) & 1 == 1;
                        for sub in 0..4usize {
                            ...
                            for j in 0..4usize {
                                for i in 0..4usize {
                                    ...
                                    recon[cy * chroma_width + cx] = (p + r).clamp(0, 255) as u8;
                                }
                            }
                        }
                    }
                ...
                for j in 0..16usize {
                    for i in 0..8usize {
                        ...
                        let res_u = if cbp_chroma == 0 {
                            ...
                            u_recon_res_422[j * 8 + i]
                        };
                        let res_v = if cbp_chroma == 0 {
                            0
                        } else {
                            v_recon_res_422[j * 8 + i]
                        };
        ...
        // §7.3.5 — mb_qp_delta is present iff the MB has coded
        // residual (cbp != 0). Only then does the decoder's QP_Y
        // advance to this MB's QP; a cbp==0 MB transmits no delta and
        // keeps the predecessor QP (its dequant is a no-op anyway).
            ...
            3 => {
                ...
                crate::encoder::macroblock::write_p_l0_16x16_mb_chroma(
                    ...
                    crate::encoder::macroblock::ChromaWriteKind::Yuv444Inter {
                        ...
                        cr_levels: &cr444.levels_scan,
                        cb_nc: &cb_nc,
                        cr_nc: &cr_nc,
                        cbp_luma,
                    },
                )
                .expect("write P_L0_16x16 mb (4:4:4)");
            }
            2 => {
                let (nc_cb, nc_cr) = derive_chroma_ac_nc_and_commit_totals(
                    nc_grid,
                ...
                .expect("write P_L0_16x16 mb (4:2:2)");
            }
            _ => {
                let (chroma_nc_cb, chroma_nc_cr) = derive_chroma_ac_nc_and_commit_totals(
                    nc_grid,
        ...
        *mv_grid.slot_mut(mb_x, mb_y) = MvGridSlot {
        ...
        // Per-4x4 nonzero mask for the §8.7 deblock walker (4:2:0 —
        // 4 blocks per plane at bits 0..=3 / 8..=11; 4:2:2 — 8 blocks
        // at bits 0..=7 / 8..=15; 4:4:4 — deblocking disabled, mask
        // unused).
        let luma_nonzero_4x4 = luma_nz_mask_from_blocks(&blk_has_nz);
        let chroma_nonzero_4x4: u16 = match chroma_array_type {
            ...
            2 => {
                let mut m = 0u16;
                for blk in 0..8usize {
                    if cbp_chroma == 2 && u_ac8[blk].iter().any(|&v| v != 0) {
                        m |= 1 << blk;
                    }
                    if cbp_chroma == 2 && v_ac8[blk].iter().any(|&v| v != 0) {
                        m |= 1 << (8 + blk);
                    }
                }
                m
            ...
            _ => {
                let cb_nz: [bool; 4] = std::array::from_fn(|i| {
                    cbp_chroma == 2 && u_ac_levels[i].iter().any(|&v| v != 0)
                });
                let cr_nz: [bool; 4] = std::array::from_fn(|i| {
                    cbp_chroma == 2 && v_ac_levels[i].iter().any(|&v| v != 0)
                });
                chroma_nz_mask_from_blocks(&cb_nz, &cr_nz)
            }
        };
        ...
        MbDeblockInfo {
            ...
            // residual (delta emitted above) and the inherited QP when
            // cbp == 0.
            qp_y: qp_tracker.cur,
            luma_nonzero_4x4,
            chroma_nonzero_4x4,
    ...
    fn encode_p_mb_4mv(
        ...
        *pending_skip = 0;
        ...
        let mb_cfg = P8x8AllPL08x8McbConfig {
            ...
            cbp_luma,
            cbp_chroma,
            // §7.3.5 — present iff cbp != 0; see encode_p_mb.
            mb_qp_delta: {
    ...
    fn encode_p_mb_with_intra_fallback(
        ...
        // 4:2:2 / 4:4:4 P-slices take the pure-inter path.
        if !self.cfg.intra_in_inter || self.cfg.chroma_format_idc != 1 {
        ...
        // The trial writes advance the §7.4.5 QP chain; snapshot it
        // alongside the other rollback state.
    ...
    fn encode_p_mb_intra16x16(
        ...
        *pending_skip = 0;
        ...
        // decoder's current QP_Y to this MB's QP and advance the
        // tracker.
        ...
        *mv_grid.slot_mut(mb_x, mb_y) = MvGridSlot {
        ...
        MbDeblockInfo {
            is_intra: true,
            // Intra_16x16 always emitted mb_qp_delta, so the chain QP
            // equals qp_y here.
            qp_y: qp_tracker.cur,
            luma_nonzero_4x4,
            chroma_nonzero_4x4: 0,
...
impl Encoder {
    ...
    pub fn encode_b_with_qp(
        ...
        // post-filter samples). 4:4:4 skips the pass (slice header set
        // disable_deblocking_filter_idc = 1).
        if self.cfg.chroma_format_idc != 3 {
            crate::encoder::deblock::deblock_recon_with_chroma_array_type(
                self.cfg.width,
    ...
    /// Round-397 — encode one B-slice MB at **4:2:2 / 4:4:4** (CAVLC).
    ///
    ...
    ///     1/8-pel); §8.5.11.2 2x4-Hadamard residual chain.
    ///   * 4:4:4 — the §8.4.2.2.1 LUMA 6-tap process per chroma plane
    ///     with mvC = mvL; §7.3.5.3 coded-like-luma residual with the
    ///     shared CBP quadrant bits (Table 9-4(b)).
    #[allow(clippy::too_many_arguments)]
    fn encode_b_mb_multifmt(
        &self,
        frame: &YuvFrame<'_>,
        ref_l0: &EncodedFrameRef<'_>,
        ref_l1: &EncodedFrameRef<'_>,
        ...
        let mb_addr = mb_y * width_mbs + mb_x;
        let chroma_w = chroma_width as u32;
        let chroma_h = _chroma_height as u32;

        // 1. Per-list quarter-pel ME — same routine as encode_b_mb.
        let me_l0 = search_quarter_pel_16x16(
            frame.y,
            src_stride,
            self.cfg.width,
        ...
        // 2. Per-format predictor builders. Luma is shared with the
        ...
        // Merge two per-list predictor tiles per §8.4.2.3.1 (default
        // average). Explicit weighted bipred (round-26) applies to the
        // LUMA merge only, mirroring `build_b_predictors`.
        let build_luma = |kind: BPred16x16, l0mv: Mv, l1mv: Mv| -> [i32; 256] {
            match kind {
                BPred16x16::L0 => build_inter_pred_luma(
                    ref_l0.recon_y,
                    ref_l0.width,
                    ref_l0.height,
                    mb_x,
                ...
                BPred16x16::L1 => build_inter_pred_luma(
                    ref_l1.recon_y,
                    ref_l1.width,
                    ref_l1.height,
                    mb_x,
                ...
                BPred16x16::Bi => {
                    ...
                    if let Some(wp) = weighted {
                        ...
                        for i in 0..256 {
                            let v =
                                (a[i] * wp.weight_l0 + b[i] * wp.weight_l1 + round) >> (logwd + 1);
                            out[i] = (v + off_avg).clamp(0, 255);
                        }
                    } else {
                        for i in 0..256 {
                            out[i] = (a[i] + b[i] + 1) >> 1;
        ...
        let build_chroma = |kind: BPred16x16, l0mv: Mv, l1mv: Mv| -> (Vec<i32>, Vec<i32>) {
            match kind {
                BPred16x16::L0 => (
                    chroma_plane_pred(ref_l0.recon_u, l0mv),
                    chroma_plane_pred(ref_l0.recon_v, l0mv),
                ),
                BPred16x16::L1 => (
                    chroma_plane_pred(ref_l1.recon_u, l1mv),
                    chroma_plane_pred(ref_l1.recon_v, l1mv),
                ),
                BPred16x16::Bi => {
                    let au = chroma_plane_pred(ref_l0.recon_u, l0mv);
                    let bu = chroma_plane_pred(ref_l1.recon_u, l1mv);
                    let av = chroma_plane_pred(ref_l0.recon_v, l0mv);
                    let bv = chroma_plane_pred(ref_l1.recon_v, l1mv);
                    (
                        au.iter()
                            .zip(&bu)
                            .map(|(&x, &y)| (x + y + 1) >> 1)
        ...
        let pred_y_l0 = build_luma(BPred16x16::L0, mv_l0, mv_l1);
        let pred_y_l1 = build_luma(BPred16x16::L1, mv_l0, mv_l1);
        let pred_y_bi = build_luma(BPred16x16::Bi, mv_l0, mv_l1);
        let sad_l0 = sad_16x16_pred(frame.y, src_stride, mb_x, mb_y, &pred_y_l0);
        let sad_l1 = sad_16x16_pred(frame.y, src_stride, mb_x, mb_y, &pred_y_l1);
        let sad_bi = sad_16x16_pred(frame.y, src_stride, mb_x, mb_y, &pred_y_bi);

        // 2.5 Direct-mode trial (uniform-derivation gate, round 21).
        let direct = if self.cfg.direct_temporal_mv_pred {
            b_temporal_direct_derive(
                ref_l1.partition_mvs,
                width_mbs,
        ...
        let (direct_competitive, direct_pred_y, direct_sad) =
            match (direct.is_uniform(), direct_pred_kind) {
                (true, Some(dpk)) => {
                    let dy = build_luma(dpk, direct.mv_l0(), direct.mv_l1());
                    let dsad = sad_16x16_pred(frame.y, src_stride, mb_x, mb_y, &dy);
                    (true, dy, dsad)
                }
                _ => (false, [0i32; 256], u32::MAX),
            };

        // 3. Mode pick — same Lagrangian direct bias as encode_b_mb.
        let direct_bias_sad: u32 = {
            let lambda = (qp_y / 6).clamp(0, 8) as u32;
            (lambda + 1) * 16
        };
        ...
        let (pred, pred_y, eff_mv_l0, eff_mv_l1) = if is_direct {
            (
                direct_pred_kind.expect("Direct chosen → direct_pred_kind set"),
                direct_pred_y,
                direct.mv_l0(),
            ...
            (BPred16x16::Bi, pred_y_bi, mv_l0, mv_l1)
        } else if sad_l0 <= sad_l1 {
            (BPred16x16::L0, pred_y_l0, mv_l0, Mv::ZERO)
        } else {
        ...
        // 6. Chroma residual — per-format dispatch (mirror of the
        //    round-397 encode_p_mb structure).
        let mut u_dc8 = [0i32; 8];
        let mut v_dc8 = [0i32; 8];
        let mut u_ac8 = [[0i32; 16]; 8];
        let mut v_ac8 = [[0i32; 16]; 8];
        ...
        match chroma_array_type {
            3 => {
                ...
                // §7.4.5.1 at ChromaArrayType == 3 — shared CBP quadrant
                // bits cover all three planes.
                for blk8 in 0..4usize {
                    let cb_nz = (0..4).any(|sub| cb444.blk_has_nz[blk8 * 4 + sub]);
                    let cr_nz = (0..4).any(|sub| cr444.blk_has_nz[blk8 * 4 + sub]);
                    if cb_nz || cr_nz {
        ...
        // Normalise the second-gate flag when nothing is coded.
        if cbp_luma == 0 && self.cfg.transform_8x8 {
            transform_size_8x8 = false;
        }
        // Flag value handed to the writer (after the 4:4:4 CBP merge /
        // normalisation): absent when the PPS doesn't carry the tool.
        let b_t8x8_flag = if self.cfg.transform_8x8 {
            Some(transform_size_8x8)
        } else {
            None
        ...
        // 7. Reconstruction (pred + residual on coded quadrants).
        let width = self.cfg.width as usize;
        for blk8 in 0..4usize {
            let send = (cbp_luma >> blk8) & 1 == 1;
            for sub in 0..4usize {
        ...
        // 8. CAVLC neighbour-grid update + per-block nC. All plane
        //    totals start at 0; the emit branch below re-commits the
        //    real values for coded MBs (a skipped MB keeps the zeros,
        //    matching §9.2.1.1 step 6).
        let mb_is_skip = is_direct && cbp_luma == 0 && cbp_chroma == 0;
        {
            let cur = &mut nc_grid.mbs[mb_addr];
            cur.is_available = true;
        ...
        // 9. Emit the MB syntax (B_Skip folds into `pending_skip`).
        if mb_is_skip {
            *pending_skip += 1;
        } else {
            sw.ue(*pending_skip);
            *pending_skip = 0;
            ...
            let emit = if chroma_array_type == 3 {
                BChromaEmit::C444 {
                    ...
                    cr_nc: derive_plane_nc_444_inter(
                        nc_grid,
                        mb_addr as u32,
                        true,
                        cbp_luma,
            ...
            if is_direct {
                ...
                crate::encoder::macroblock::write_b_direct_16x16_mb_chroma(
                    sw,
                    &dcfg,
                    chroma_array_type,
                    chroma_kind,
                ...
                crate::encoder::macroblock::write_b_16x16_mb_chroma(
                    sw,
                    &mb_cfg,
                    0,
                    0,
        ...
        *mv_grid_l1.slot_mut(mb_x, mb_y) = if l1_used {
        ...
        // 11. §8.7 deblock info. 4:2:2 packs the 8-block per-plane AC
        //     mask (bits 0..=7 Cb, 8..=15 Cr); 4:4:4 has deblocking
    ...
    fn encode_b_mb_with_intra_fallback(
        ...
        // Round-397 — the Intra_16x16-in-inter trial body is
        // 4:2:0-only (its chroma trial + snapshot windows are 8x8);
        // 4:2:2 / 4:4:4 B-slices take the pure-inter path.
        if !self.cfg.intra_in_inter || self.cfg.chroma_format_idc != 1 {
