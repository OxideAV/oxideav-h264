...
Intra_16×16 DC_PRED output stream. **High-Profile 8×8 transform** is
wired end-to-end on both the CAVLC path (§7.3.5.3.2, §8.5.13, bit-exact
against libavcodec) and the CABAC path (§9.3.3.1.1.10 for the transform
size flag + §9.3.3.1.1.9 ctxBlockCat = 5 for the 64-coefficient residual).
Zero C dependencies.
...
- **Decoder**: CABAC B-slices (CAVLC B works). CABAC P-slices are wired
  ...
  P_8×8 with all four sub-partitions, intra-in-P), including
  `transform_size_8x8_flag = 1` on both intra and inter paths. Weighted
  prediction on the CABAC P path is parsed but weights are not applied;
  encoders should disable weighted P for CABAC bitstreams fed into this
  decoder, or tolerate unweighted MC output.
