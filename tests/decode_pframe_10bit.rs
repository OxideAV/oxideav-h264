...
fn decode_10bit_pframes_against_reference() {
    ...
    for (i, frame) in frames.iter().enumerate() {
        ...
        let ref_cb = unpack_u16_le(&yuv[ref_off + y_plane_bytes..ref_off + y_plane_bytes + c_plane_bytes]);
        let ref_cr = unpack_u16_le(&yuv[ref_off + y_plane_bytes + c_plane_bytes..ref_off + frame_bytes]);
