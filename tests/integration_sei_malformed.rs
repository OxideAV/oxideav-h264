...
/// Mirror of `parse_payload`'s dispatch arms — 47 implemented types
/// (round 231 adds Annex H §H.13.2.3 depth_representation_info type 50
/// on top of the round-226 Annex H §H.13.2.4
...
const KNOWN_PAYLOAD_TYPES: &[u32] = &[
    ...
    41, 43, 45, 46, 47, 50, 51, 137, 142, 144, 147, 148, 149, 150, 151, 154, 155, 156, 200, 201,
    205,
];

const FALLBACK_PAYLOAD_TYPES: &[u32] = &[
    24, 30, 36, 54, 56, 100, 138, 140, 152, 153, 157, 160, 199, 202, 204, 206, 255, 4096, 65535,
    100_000, 1_000_000,
];
...
fn sei_parse_payload_never_panics() {
    ...
    // 47 known + 21 fallback = 68 payload types × 11 shapes × 4 ctxs
    // = 2992 invocations. Lock the count so a future change that
    // accidentally drops a row from one of the tables makes the test
    // fail loudly instead of silently shrinking coverage. (Round 231
    // lifts type 50 depth_representation_info out of the fallback
    // list into the implemented group; the per-type total is
    // unchanged at 68 because exactly one type migrated.)
