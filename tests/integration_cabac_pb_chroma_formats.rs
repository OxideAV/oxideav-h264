...
// ---------------------------------------------------------------------------
// Round-391 — CABAC B-slices at 4:2:2 / 4:4:4 (16x16 explicit mode set).
// ---------------------------------------------------------------------------
...
fn assert_gop_b_ffmpeg_interop(gop: &GopB, fmt: u32, tag: &str) {
    ...
    let tmpdir = std::env::temp_dir();
    let h264_path = tmpdir.join(format!(
        "oxideav_h264_r391_cabb_{tag}_{}.h264",
        std::process::id()
    ));
    let yuv_path = tmpdir.join(format!(
        "oxideav_h264_r391_cabb_{tag}_{}.yuv",
        std::process::id()
    ));
    std::fs::write(&h264_path, &gop.combined).expect("write h264");
    let status = std::process::Command::new(ffmpeg)
...
#[test]
fn cabac_b_422_gop_bit_exact() {
    let gop = encode_gop_b(2, false);
    assert!(gop.b.annex_b.len() > 100, "B too small — likely all-zero");
    assert_gop_b_self_roundtrip(&gop, 2, "cabac-b-422");
    assert_gop_b_ffmpeg_interop(&gop, 2, "cabac-b-422");
}

#[test]
fn cabac_b_444_gop_bit_exact() {
    let gop = encode_gop_b(3, false);
    assert!(gop.b.annex_b.len() > 100, "B too small — likely all-zero");
    assert_gop_b_self_roundtrip(&gop, 3, "cabac-b-444");
    assert_gop_b_ffmpeg_interop(&gop, 3, "cabac-b-444");
}

#[test]
fn cabac_b_422_transform_8x8_gop_bit_exact() {
    let gop = encode_gop_b(2, true);
    assert_gop_b_self_roundtrip(&gop, 2, "cabac-b-422-8x8");
    assert_gop_b_ffmpeg_interop(&gop, 2, "cabac-b-422-8x8");
}

#[test]
fn cabac_b_444_transform_8x8_gop_bit_exact() {
