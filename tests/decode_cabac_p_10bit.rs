//! 10-bit CABAC P-slice decode — compares the decoded u16 planes against
//! the ffmpeg `-pix_fmt yuv420p10le` reference YUV shipped under
//! `tests/fixtures/10bit_cabac_p_64x64.{es,yuv}`.
//!
...
//!   -coder 1 -x264-params "bframes=0:keyint=3:no-scenecut=1" \
//!   /tmp/10bit_cabac_p.mp4
//! ffmpeg -i /tmp/10bit_cabac_p.mp4 -c copy -bsf:v h264_mp4toannexb \
//!   -f h264 tests/fixtures/10bit_cabac_p_64x64.es
//! ffmpeg -i /tmp/10bit_cabac_p.mp4 -f rawvideo -pix_fmt yuv420p10le \
//!   tests/fixtures/10bit_cabac_p_64x64.yuv
//! ```
...
#[test]
fn decode_10bit_cabac_pframes_against_reference() {
    let es_path = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/tests/fixtures/10bit_cabac_p_64x64.es"
    );
    let yuv_path = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/tests/fixtures/10bit_cabac_p_64x64.yuv"
    ...
    assert_eq!(frames.len(), 3, "expected 3 decoded frames (IDR + 2 P)");
    for f in &frames {
    ...
    for (i, frame) in frames.iter().enumerate() {
        ...
        eprintln!(
            "10bit CABAC P frame #{i}: bit-exact {}/{} ({:.2}%); within ±4 LSB {}/{} ({:.2}%)",
            frame_exact,
            frame_total,
    ...
    eprintln!(
        "10bit CABAC P (all 3 frames): bit-exact {}/{} ({:.2}%); within ±4 LSB {}/{} ({:.2}%)",
        total_exact, total, exact_pct, total_within4, total, within4_pct,
    );

    assert!(
        exact_pct >= 99.0,
        "10bit CABAC P bit-exact rate {exact_pct:.2}% < 99% — regression"
    );
}
